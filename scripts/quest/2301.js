...
function start(mode, type, selection) {
	...
	case 3:
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.ETC).getNumFreeSlot() < 1) {
			...
			Packages.server.quest.MapleQuest.getInstance(2301).forceStart(qm.getPlayer(), qm.getNpc(), null);
...
function end(mode, type, selection) {
	...
	case 2:
		...
		break;
	case 3:
		Packages.server.quest.MapleQuest.getInstance(2301).forceComplete(qm.getPlayer(), qm.getNpc());
