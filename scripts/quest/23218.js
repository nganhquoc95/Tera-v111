...
function start(mode, type, selection) {
	...
	case 2:
		if (qm.getMap(931050120).getCharacters().size() < 1) {
			Packages.server.quest.MapleQuest.getInstance(23218).forceStart(qm.getPlayer(), qm.getNpc(), null);
			qm.getMap(931050120).resetFully();
			qm.getPlayer().changeMap(qm.getMap(931050120), qm.getMap(931050120).getPortal(1));
			qm.getPlayer().getMap().spawnMonsterOnGroundBelow(Packages.server.life.MapleLifeFactory.getMonster(9001037), new java.awt.Point(714, -14));
			qm.dispose();
			return;
			}
...
function end(mode, type, selection) {
	...
	case 0:
		...
		break;
	case 1:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(23218)).getStatus() < 2) {
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 3) {
			...
			Packages.server.quest.MapleQuest.getInstance(23218).forceComplete(qm.getPlayer(), qm.getNpc());
			}
