...
function end(mode, type, selection) {
	...
	switch (status) {
	case 0:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(1453)).getStatus() < 1) {
			Packages.server.quest.MapleQuest.getInstance(1453).forceStart(qm.getPlayer(), qm.getNpc(), null);
			qm.dispose();
			return;
			}
			...
			break;
	case 1:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(1453)).getStatus() < 2) {
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			qm.sendNext("You don't have enough space in your Inventory. Make some room and try again.");
			qm.dispose();
			return;
			}
			Packages.server.quest.MapleQuest.getInstance(1453).forceComplete(qm.getPlayer(), qm.getNpc());
