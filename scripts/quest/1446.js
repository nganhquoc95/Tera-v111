...
function end(mode, type, selection) {
	...
	case 0:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(1446)).getStatus() < 1) {
			...
			break;
	case 1:
		qm.sendYesNo("The fight with the true Pirate, Kyrin, has give you the strength of a true Pirate. Can you feel it? All that is left is the Job Advancement. Are you prepared to become an Outlaw, an even stronger Pirate?");
		break;
	case 2:
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			...
			qm.sendOk("You are now a #bOutlaw#k, the master of #bfists and knuckles#k. As a true Outlaw, use your strength to the fullest!");
			break;
	case 3:
		qm.dispose();
}
