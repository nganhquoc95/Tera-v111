...
function end(mode, type, selection) {
	...
	case 0:
		if (qm.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(1432)).getStatus() < 1) {
			...
			break;
	case 1:
		qm.sendYesNo("By fighting against #bDances with Balrog#k, you have proven your right to become a true Warrior. You may now take the title of #bWhite Knight#k...are you ready?");
		break;
	case 2:
		if (qm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).getNumFreeSlot() < 1) {
			...
			qm.sendOk("You are now a #bWhite Knight#k, a master of #bswords and blunt weapons#k. As a true Warrior, show the world your strength!");
			break;
	case 3:
		qm.dispose();
}
