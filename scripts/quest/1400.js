/*
	名字:	五種冒險的方法
	地圖:	冒險者修練場入口
	描述:	1010000
*/

var status = -1;

function start(mode, type, selection) {
	switch (mode) {
	case -1:
		qm.dispose();
		return;
	case 0:
		if (status < 2) {
		qm.dispose();
		return;
		}
		status--;
		break;
	case 1:
		status++;
		break;
		}
	switch (status) {
	case 0:
		qm.sendNext("Hmm, you're making good progress with your leveling. Have you decided on which job you want to take? You could be a Warrior with great strength and high HP, a Magician with many spells, a Bowman that shoots arrows from afar, a Thief that uses quick, sneaky attacks, or a Pirate with all kinds of flashy chain skills... There are so many!");
		break;
	case 1:
		var chat = "If you go to Victoria Island, you can advance to the job of your choice by going to the right Job Instructor. But before that, lemme know which one you're interested in, and I'll send #bthem#k a letter of recommendation. That will make it easier for you to advance! So, which job will you choose?#b";
		chat += "\r\n#L1#I want to be a might Warrior!";
		chat += "\r\n#L2#I want to be a mystical Magician!";
		chat += "\r\n#L3#I want to be a sharp-shooting Bowman!";
		chat += "\r\n#L4#I want to be a sneaky Thief!";
		chat += "\r\n#L5#I want to be a swashbuckling Pirate!";
		qm.sendSimple(chat);
		break;
	case 2:
		if (selection == 1)
			qm.sendNext("A Warrior, huh? Boy, you're going to get really strong! They can take tons of damage, and dish plenty out, too. Okay, I'll send my recommendation to #bDances with Balrog#k, the Warrior Job Instructor.");
		if (selection == 2)
			qm.sendNext("You want to be a Magician? They sure are mysterious! Their magic is super pwerful and has all kinds of effects. Just don't get hit...Magicians aren't known for their endurance! Okay, I'll send my recommendation to #bGrendel the Really Old#k, the Magician Job Instructor.");
		if (selection == 3)
			qm.sendNext("You wanna be a Bowman? I hope you have really good aim! With their great dexterity, they have no problem avoiding attacks and firing off plenty of their own. Okay, I'll send my recommendation to #bAthena Pierce#k, the Bowman. Job Instructor.");
		if (selection == 4)
			qm.sendNext("Going to be a Thief, are you? They're so quick and sneaky, their enemies don't see them coming until it's too late. They're so cool! Okay, I'll send my recommendation letter to #bDark Lord#k, the Thief Job Instructor.");
		if (selection == 5) {
			qm.sendNext("A Pirate? Yarr! Whether in a gunfight or a hand-to-hand brawl, Pirates fight with style! I think you're up to the challenge. Okay, I'll send my recommendation to #bKyrin#k, the Pirate Job Instructor.");
			}
			Packages.server.quest.MapleQuest.getInstance(1406).forceStart(qm.getPlayer(), qm.getNpc(), selection);
			Packages.server.quest.MapleQuest.getInstance(1400).forceComplete(qm.getPlayer(), qm.getNpc());
			qm.getClient().getSession().write(Packages.tools.packet.MaplePacketCreator.getShowQuestCompletion(1400));
			select = selection;
			break;
	case 3:
		if (select == 1)
			qm.sendOk("He will contact when you reach Lv. 10. Become a great Warrior!");
		if (select == 2)
			qm.sendOk("You know that Magicians have their Job Advancements earlier than other jobs, right? Grendel the Really Old will contact you once you reach Lv. 8. Become an amazing Magician!");
		if (select == 3)
			qm.sendOk("She'll contact you once you reach #bLv. 10#k. I hope you become a magnificent Bowman!");
		if (select == 4)
			qm.sendOk("If you get up to #bLv. 10#k, he'll contact you. Become a great Thief, got it?");
		if (select == 5) {
			qm.sendOk("She'll contact you once you reach #bLv. 10#k. Become a masterful Pirate!");
			}
			qm.dispose();
}
}