...
public class MapleItemInformationProvider {
    ...
    public void runEtc() {
        ...
        for (MapleData dat : potsData) {
            ...
            for (MapleData potLevel : dat.getChildByPath("level")) {
                ...
                item.stringTemplate = MapleDataTool.getString("info/string", dat, "");
            ...
            if (GameConstants.isPotentialScroll(scrollId.getItemId()) || GameConstants.isEquipScroll(scrollId.getItemId()) || GameConstants.isSpecialScroll(scrollId.getItemId()) || Randomizer.nextInt(100) <= success) {
                switch (scrollId.getItemId()) {
                    ...
                    case 2040727: { // Spikes on shoe, prevents slip
                        nEquip.addAttribute(ItemFlag.SPIKES.getValue());
                        break;
                    }
                    case 2041058: { // Cape for Cold protection
                        nEquip.addAttribute(ItemFlag.COLD.getValue());
                        break;
                    }
                    case 5063000:
                    case 2530000:
                    case 2530001: {
                        nEquip.addAttribute(ItemFlag.LUKCYDAY.getValue());
                        break;
                    }
                    case 5064000:
                    case 2531000: {
                        nEquip.addAttribute(ItemFlag.PROTECT.getValue());
                        break;
                    }
                    default: {
    ...
    public ArrayList<ItemInformation> getItemDataByName(String name) {
        ArrayList<ItemInformation> ret = new ArrayList<>();
        MapleItemInformationProvider.getInstance().getAllItems().stream().filter(itemPair -> (itemPair.name.toLowerCase().contains(name.toLowerCase()))).forEachOrdered(itemPair -> {
            ret.add(itemPair);
        });
        return ret;
    }
